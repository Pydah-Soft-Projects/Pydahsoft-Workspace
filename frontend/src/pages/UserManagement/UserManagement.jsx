...
const PAGE_PRIVILEGES_LIST = [
  { key: 'canViewOverview', label: 'Dashboard Overview Page', icon: 'overview', path: '/dashboard/overview' },
  { key: 'canViewUsers', label: 'User Accounts & Credentials Page', icon: 'users', path: '/dashboard/users' },
  { key: 'canViewEmployees', label: 'Employee Directory & Staff Profiles Page', icon: 'employees', path: '/dashboard/employees' },
  { key: 'canViewProjects', label: 'Projects & Modules Page', icon: 'projects', path: '/dashboard/projects' },
  { key: 'canViewTeams', label: 'Teams & Tasks Management Page', icon: 'teams', path: '/dashboard/teams' },
  { key: 'canViewTimeTracker', label: 'Interactive Time Tracker Page', icon: 'time-tracker', path: '/dashboard/time-tracker' },
  { key: 'canViewReviews', label: 'Task Approvals & Reviews Queue Page', icon: 'reviews', path: '/dashboard/reviews' },
  { key: 'canViewDailyPlans', label: 'Daily Work Plans Page', icon: 'daily-plans', path: '/dashboard/daily-plans' },
  { key: 'canViewAnalytics', label: 'Performance Analytics & Reports Page', icon: 'analytics', path: '/dashboard/analytics' },
  { key: 'canViewAuditLogs', label: 'System Audit Trail Logs Page', icon: 'audit-logs', path: '/dashboard/audit-logs' },
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-3 md:hidden">
            {users.map((u, idx) => {
              ...
              return (
                <div key={u._id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    <div className="w-11 h-11 rounded-2xl bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-base border border-indigo-200 shrink-0 shadow-2xs">
                      {u.name ? u.name.charAt(0).toUpperCase() : 'U'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        <h4 className="text-xs font-black text-[#09233d] truncate">{u.name}</h4>
                      ...
                      <p className="text-[11px] text-gray-400 font-medium truncate mt-0.5">@{u.username}</p>

                      <div className="mt-2 text-xs font-semibold text-gray-700 flex flex-col gap-1 bg-gray-50/80 p-2.5 rounded-xl border border-gray-100">
                  ...
                  <div className="pt-2 border-t border-gray-100 flex items-center justify-between gap-1.5 overflow-x-auto">
                    ...
                    <button
                      ...
                      className="flex-1 py-1.5 px-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl text-[11px] font-bold border border-indigo-200 flex items-center justify-center gap-1 cursor-pointer transition-all active:scale-95 whitespace-nowrap"
                    >
                      <Icon name="settings" className="w-3.5 h-3.5 text-indigo-600" /> Privileges
                    </button>
