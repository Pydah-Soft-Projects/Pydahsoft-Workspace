...
export default function UserManagement({ currentUser }) {
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      if (editNewPassword.trim()) payload.newPassword = editNewPassword.trim();
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-3">
              ...
              <div className="border border-orange-200 rounded-xl p-3 bg-orange-50/60 space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-bold text-orange-700 uppercase flex items-center gap-1.5">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                    </svg>
                    Change Password
                  </label>
                  <span className="text-[9px] text-orange-500 font-semibold">Leave blank to keep current</span>
                </div>
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-orange-200 rounded-xl p-2.5 pr-10 text-xs font-medium focus:border-orange-400 focus:outline-none bg-white"
                  />
                  <button
                    type="button"
              ...
              <div className="flex justify-end gap-2 pt-3">
                <button
                  type="button"
                  onClick={() => { setEditingUser(null); setEditNewPassword(''); setShowEditPassword(false); }}
                  className="px-4 py-2 border border-gray-200 rounded-xl text-xs font-bold text-gray-600"
                >
                  Cancel
                </button>
